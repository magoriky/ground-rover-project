  ...
  node_up_(false)
{
    ...
    torque_ = false;
    max_rpm_ = 0;
    rpm_pub_ = nh->advertise<motor_driver::RobotMotor>("/cur_rpm", 100);
...
void MotorDriver::cmd_rpm_callback(const motor_driver::RobotMotor::ConstPtr &msg){
...
void MotorDriver::rpm_pub(const ros::TimerEvent &e){
    if(node_up_){
        motor_driver::RobotMotor rpm_msg;
